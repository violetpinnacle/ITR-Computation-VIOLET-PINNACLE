// Renders the ITR-4 (Sugam) computation sheet into the page in the same
// formal, CA-software-style layout used for ITR-1, but with an added
// "Presumptive Business / Professional Income" section.
...
// Only prints a presumptive-income sub-row if that section has any turnover
// or income reported, so unused sections (e.g. no 44AE vehicles) stay hidden.
function presumptiveRow(label, turnover, income) {
  if (!turnover && !income) return '';
  return `
    <tr><td class="indent">${label} - Turnover / Gross Receipts</td><td class="amount">${formatCurrency(turnover)}</td></tr>
    <tr><td class="indent">${label} - Presumptive Income</td><td class="amount">${formatCurrency(income)}</td></tr>
  `;
}

function renderComputationITR4(parsed, computed) {
  ...
  const ayRaw = parseInt(parsed.filing.assessmentYear);
  const ayLabel = isNaN(ayRaw) ? parsed.filing.assessmentYear : `${ayRaw}-${(ayRaw + 1).toString().slice(-2)}`;
  const finYear = isNaN(ayRaw) ? '' : `${ayRaw - 1}-${ayRaw.toString().slice(-2)}`;

  const businessSection = presumptiveRow('Section 44AD (Business)', parsed.income.presumptive44AD.grossTurnover, parsed.income.presumptive44AD.presumptiveIncome)
    + presumptiveRow('Section 44ADA (Profession)', parsed.income.presumptive44ADA.grossReceipts, parsed.income.presumptive44ADA.presumptiveIncome)
    + presumptiveRow('Section 44AE (Goods Carriage)', 0, parsed.income.presumptive44AE.presumptiveIncome);

  container.innerHTML = `
    <div class="comp-sheet" id="printArea" oncopy="return false" oncontextmenu="return false" onselectstart="return false">
      <div class="sheet-title">
        <h2>Computation of Total Income and Tax Payable (ITR-4 / Sugam)</h2>
      ...
      <table class="info-table">
        ...
        <tr>
          <td class="info-label">Status</td><td>:</td><td>${parsed.personal.status}</td>
      ...
      <table class="comp-table">
        ...
        <tr><td colspan="2"><strong>Presumptive Income from Business / Profession</strong></td></tr>
        ${businessSection}
        ${lineRow('Income from Business / Profession (Presumptive)', parsed.income.incomeFromBusiness, { bold: true })}
      ...
      <p class="disclaimer">
        This is an unofficial computation sheet generated for reference purposes only, based on the
        presumptive income declared under Sections 44AD / 44ADA / 44AE. It is not a substitute for the
        official filed return or professional tax advice.
      </p>
    </div>
...
function downloadAsWord() {
  ...
  const html = `
    ...
    <head><meta charset="utf-8"><title>ITR-4 Computation Sheet</title><style>${styles}</style></head>
  ...
  a.href = url;
  a.download = 'ITR4-Computation-Sheet.doc';
